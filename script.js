...
const REFRESH_RATE_LIMIT = {
  maxRequests: 5,           // Maximum refreshes allowed
  windowMs: 5 * 60 * 1000,  // 5 minute window
  cooldownMs: 30 * 1000     // 30 second cooldown between refreshes
};
...
function canRefresh() {
  const now = Date.now();
  const windowStart = now - REFRESH_RATE_LIMIT.windowMs;
  
  // Remove old requests outside the time window
  refreshRateLimit.requests = refreshRateLimit.requests.filter(
    timestamp => timestamp > windowStart
  );
  
  // Check cooldown period
  const timeSinceLastRefresh = now - refreshRateLimit.lastRefresh;
  if (timeSinceLastRefresh < REFRESH_RATE_LIMIT.cooldownMs) {
    return {
      allowed: false,
      reason: 'cooldown',
      waitTime: REFRESH_RATE_LIMIT.cooldownMs - timeSinceLastRefresh
    };
  }
  
  // Check rate limit
  if (refreshRateLimit.requests.length >= REFRESH_RATE_LIMIT.maxRequests) {
    const oldestRequest = Math.min(...refreshRateLimit.requests);
    const resetTime = oldestRequest + REFRESH_RATE_LIMIT.windowMs;
    return {
      allowed: false,
      reason: 'rateLimit',
...
function updateRefreshButton() {
  ...
  if (refreshCheck.allowed) {
    ...
    const waitSeconds = Math.ceil(refreshCheck.waitTime / 1000);
    const waitMinutes = Math.floor(waitSeconds / 60);
    const remainingSeconds = waitSeconds % 60;
    
    let waitText;
    if (waitMinutes > 0) {
      waitText = `${waitMinutes}m ${remainingSeconds}s`;
    } else {
      waitText = `${waitSeconds}s`;
    }
    
    if (refreshCheck.reason === 'cooldown') {
      status.textContent = `Wait ${waitText} between refreshes`;
    } else {
      status.textContent = `Rate limit reached. Try again in ${waitText}`;
    }
    
    status.className = 'refresh-status error';
    
    // Set a timeout to re-check when the wait period expires
    setTimeout(updateRefreshButton, Math.min(refreshCheck.waitTime, 1000));
  }
}
...
async function handleRefresh() {
  ...
  const activeButton = bossButtonsDiv?.querySelector('button.active');
  if (!activeButton) return;
  
  const encounterId = parseInt(activeButton.dataset.encounterId);
  const bossName = activeButton.dataset.bossName;
  
  if (!encounterId || !bossName) return;
  
  // Update rate limiter state
  const now = Date.now();
  refreshRateLimit.requests.push(now);
  refreshRateLimit.lastRefresh = now;
  ...
  try {
    ...
    await fetchAndDisplayRankings(bossName, encounterId, { force: true });
    ...
    if (status) {
      ...
      setTimeout(() => {
        if (status.className === 'refresh-status success') {
          status.textContent = '';
          status.className = 'refresh-status';
        }
      }, 3000);
...
function initializeRefreshButton() {
  ...
  if (button) {
    ...
    // Update button state every second to handle countdowns
    setInterval(updateRefreshButton, 1000);
  }
}

// Reset rate limit data on page load (optional - remove if you want limits to persist across sessions)
function resetRateLimit() {
  const stored = localStorage.getItem('refreshRateLimit');
  if (stored) {
    try {
      const parsed = JSON.parse(stored);
      // Only restore if the data is recent (within the time window)
      const now = Date.now();
      const recentRequests = parsed.requests?.filter(
        timestamp => timestamp > now - REFRESH_RATE_LIMIT.windowMs
      ) || [];
      
      refreshRateLimit.requests = recentRequests;
      refreshRateLimit.lastRefresh = parsed.lastRefresh || 0;
    } catch (e) {
      // If parsing fails, start fresh
      refreshRateLimit.requests = [];
      refreshRateLimit.lastRefresh = 0;
    }
  }
}

// Save rate limit state to localStorage (to persist across page reloads)
function saveRateLimit() {
  try {
    localStorage.setItem('refreshRateLimit', JSON.stringify({
      requests: refreshRateLimit.requests,
      lastRefresh: refreshRateLimit.lastRefresh
    }));
  } catch (e) {
...
// Save rate limit state periodically and on page unload
setInterval(saveRateLimit, 5000); // Save every 5 seconds
window.addEventListener('beforeunload', saveRateLimit);
